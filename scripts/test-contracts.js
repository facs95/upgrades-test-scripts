...
class ContractTester {
    ...
    async runTest(testName, testFunction) {
        ...
        try {
            ...
            if (this.verbose && result !== undefined) {
                this.log(`  Result: ${JSON.stringify(result, (key, value) =>
    ...
    async testBasicContractOperations() {
        ...
        await this.runTest("Increment counter", async () => {
            ...
            if (newCounter !== initialCounter + 1n) {
                throw new Error(`Counter not incremented correctly. Expected ${initialCounter + 1n}, got ${newCounter}`);
            }
    ...
    async testEventEmissions() {
        ...
        await this.runTest("Counter increment event", async () => {
            ...
            const counterEvent = events.find(event => event.name === 'CounterIncremented');
        ...
        await this.runTest("Message update event", async () => {
            ...
            const messageEvent = events.find(event => event.name === 'MessageUpdated');
    ...
    async testComplexDataStructures() {
        ...
        await this.runTest("Array operations", async () => {
            ...
            await contract.addToArray(100);
            await contract.addToArray(200);
            await contract.addToArray(300);
    ...
    async testERC20Operations() {
        ...
        await this.runTest("ERC20 approve and transferFrom", async () => {
            ...
            // Approve
            await token.approve(recipient.address, approveAmount);
            const allowance = await token.allowance(signer.address, recipient.address);

            // TransferFrom (using recipient as spender)
            const tokenAsRecipient = token.connect(recipient);
            const tx = await tokenAsRecipient.transferFrom(signer.address, recipient.address, transferAmount);
            const receipt = await tx.wait();

            const newAllowance = await token.allowance(signer.address, recipient.address);
        ...
        await this.runTest("Force revert with reason", async () => {
            try {
                ...
                if (error.message.includes("Test revert message")) {
                    return { result: "Revert caught correctly", reason: "Test revert message" };
                }
                throw error;
            }
        });
    ...
    async runAllTests() {
        this.log(`${COLORS.BRIGHT}${COLORS.BLUE}Starting comprehensive contract testing...${COLORS.RESET}`);
        ...
        if (this.contracts.testContract) {
            ...
            this.log(`TestContract: ${await this.contracts.testContract.getAddress()}`);
        ...
        return {
            ...
            duration,
            contracts: {
                testContract: this.contracts.testContract ? await this.contracts.testContract.getAddress() : null,
                testERC20: this.contracts.testERC20 ? await this.contracts.testERC20.getAddress() : null
            }
        };
    }
}
...
async function main() {
    try {
        ...
        console.error(`${COLORS.RED}Fatal error during contract testing:${COLORS.RESET}`, error);
        process.exit(1);
    }
}
